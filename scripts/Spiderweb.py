...
def plot_sspace(data_plot,csv,frame,colvar,radar,k,normfactor,lines,line_color,cmap,lw=0.5,factor=0.00005,zord=0): #0.00001,lw=0.1,zord=0):
...
#%% Settings
threshold_E = 2000 # Lower threshold of energy capacity for the storage configuration to be included in the analysis
...
#%% Merge sub datasets
# sspace_old = pd.read_csv('../results/sspace_w_sectorcoupling.csv',index_col=0)#.fillna('0')
# csv_new_eta1 = pd.read_csv('../results/sspace_added_bounds_eta1.csv',index_col=0).loc[sspace_old.index]
# csv_new_eta2 = pd.read_csv('../results/sspace_added_bounds_eta2.csv',index_col=0).loc[sspace_old.index]
# csv_new_2 = pd.read_csv('../results/sspace_added_bounds_2.csv',index_col=0).loc[sspace_old.index]
# csv_new_3 = pd.read_csv('../results/sspace_added_bounds_3.csv',index_col=0).loc[sspace_old.index]

# csv_new_eta1_T = csv_new_eta1.T#.query('sector == "T-H"').drop(columns='sector').astype(float)
# csv_new_eta1_T['eta1 [-]'].replace({'0.975':'0.95',0.975:0.95},inplace=True)
# csv_new_eta1_T['eta2 [-]'].replace({'0.975':'0.95',0.975:0.95},inplace=True)

# csv_new_eta2_T = csv_new_eta2.T
# csv_new_eta2_T['eta1 [-]'].replace({'0.975':'0.95',0.975:0.95},inplace=True)
# csv_new_eta2_T['eta2 [-]'].replace({'0.975':'0.95',0.975:0.95},inplace=True)

# csv_new_2_T = csv_new_2.T
# csv_new_2_T['eta1 [-]'].replace({'0.975':'0.95',0.975:0.95},inplace=True)
# csv_new_2_T['eta2 [-]'].replace({'0.975':'0.95',0.975:0.95},inplace=True)

# csv_new_3_T = csv_new_3.T
# csv_new_3_T['eta1 [-]'].replace({'0.975':'0.95',0.975:0.95},inplace=True)
# csv_new_3_T['eta2 [-]'].replace({'0.975':'0.95',0.975:0.95},inplace=True)

# sspace_new_T = pd.concat([sspace_old.T,
#                           csv_new_eta1_T,
#                           csv_new_eta2_T,
#                           csv_new_2_T,
#                           csv_new_3_T],ignore_index=True)
# sspace_new = sspace_new_T.T
...
fig1.savefig('../Figures/Spiderweb_result_' + sector + '_w_' + '_' + colvar + '_Nperf' + str(N_perfections) + '.png',
