...
def plot_sspace(data_plot,csv,frame,colvar,radar,k,normfactor,lines,line_color,cmap,factor=0.0001,lw=0.1,zord=0):
...
#%% Settings
threshold_E = 2 # Lower threshold of energy capacity for the storage configuration to be included in the analysis

colorsectors = {'0':'Reds','T-H':'Purples','T-H-I-B':'Blues'}
...
csv = pd.read_csv('../Results/Config_' + str(0) + 'TWh_' + sector + '.csv',index_col=0)
...
for ex_i in ex_stores:
    ...
    for i in case.index:
        ...
        if i in csv['E_cor'][csv['E_cor'] >= threshold_E].index:
            if (ex_i == 'PTES_[1]') and (eta_1 > 1):
                ...
                plot_sspace(data_plot,csv,frame,colvar,radar,k,normfactor,lines,line_color,cmap,zord=-1) #,factor=0.0001,lw=0.1,zord=0):
            elif eta_1 <= 1:
                cmap = plt.cm.get_cmap(colorsectors[sector])
                plot_sspace(data_plot,csv,frame,colvar,radar,k,normfactor,lines,line_color,cmap) #,factor=0.0001,lw=0.1,zord=0):
            
        k += 1
    
    case1 = case.copy()
    case1['eta_c'] = eta_c_range[1] - case1['eta_c']
    case1['eta_d'] = eta_d_range[1] - case1['eta_d']
    case1['tau_SD'] = tau_SD_range[1] - case1['tau_SD']
    case1_idxmax = case1.T.product().idxmax()
    ...
    radar.fill(A1['high'],alpha=1,zorder=50,linewidth = 0.3,facecolor='None', edgecolor='gray')
    radar.fill(A1['mid'],alpha=1,zorder=50,linewidth = 0.3,facecolor='None', edgecolor='gray')
    radar.fill(A1['low'],alpha=1,zorder=50,linewidth = 2,facecolor='None', edgecolor='gray')
    
    ax.text(np.pi/2,0.03,'0',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='center')
    ax.text(np.pi/2,0.52,'20',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='center')
    ax.text(np.pi/2,0.95,'40',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='center')
    
    ax.text(np.pi/6,0.03,'1',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='center')
    ax.text(np.pi/6,0.55,'0.5',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='center')
    ax.text(np.pi/6,0.95,'0',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='center')
    
    ax.text(-np.pi/5,0.04,'0',fontsize=fs,color='gray',horizontalalignment='center')
    ax.text(-np.pi/5.5,0.54,'350',fontsize=fs,color='gray',horizontalalignment='center')
    ax.text(-np.pi/5.5,0.9,'700',fontsize=fs,color='gray',horizontalalignment='center')
    
    ax.text(-np.pi/2,0.01,'>60',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='top')
    ax.text(-np.pi/2,0.5,'30',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='top')
    ax.text(-np.pi/2,0.9,'0',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='top')
    
    ax.text(-5*np.pi/6,0.03,'1',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='center')
    ax.text(-5*np.pi/6,0.55,'0.5',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='center')
    ax.text(-5*np.pi/6,0.95,'0',fontsize=fs,color='gray',horizontalalignment='center',verticalalignment='center')
    
    ax.text(0.84*np.pi,0.04,'0',fontsize=fs,color='gray',horizontalalignment='center')
    ax.text(0.84*np.pi,0.54,'350',fontsize=fs,color='gray',horizontalalignment='center')
    ax.text(0.84*np.pi,0.9,'700',fontsize=fs,color='gray',horizontalalignment='center')
    ...
    fig1.savefig('../Figures/Spiderweb_result_' + sector + '_w_' + ex_i + '_' + colvar + '.png',
