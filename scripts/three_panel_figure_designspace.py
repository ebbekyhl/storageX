# -*- coding: utf-8 -*-
"""
Created on Thu Feb 16 08:52:27 2023
...
def plot_2D_panels(sector, slack, threshold, normfactor=100, color_variable='count_norm', combination=True, write_extra_indices = False, omit_charge_efficiency = True, lock_tau = False):
    ...
    df_cs_E = df1_update.query("E_cor >= @threshold_E")[['c1','c2',output]].groupby(['c2','c1']).min() #quantile(quantile,interpolation='nearest').copy()
    ...
    df_cs[output].loc[list(df_cs['count'][df_cs['count'] == 0].index.values)] = df1_update.groupby(['c2','c1']).max().loc[list(df_cs['count'][df_cs['count'] == 0].index.values)]['E_cor']
    ...
    for i in range(len(df_cs)):
        ...
        if np.isnan(out1) or out1 == 0:
            ...
            if combination == True:
                ...
                max_ind = df1_update.query('E_cor >= @threshold_E').groupby(['c2','c1']).max().loc[df_cs.iloc[i].name]
                min_ind = df1_update.query('E_cor >= @threshold_E').groupby(['c2','c1']).min().loc[df_cs.iloc[i].name]
    ...
    df_chat_eta2_count = df1_update.query("E_cor >= @threshold_E")[['c_hat','eta2',output]].groupby(['eta2','c_hat']).count() #quantile(quantile,interpolation='nearest').copy()
    df_chat_eta2_E = df1_update.query("E_cor >= @threshold_E")[['c_hat','eta2',output]].groupby(['eta2','c_hat']).min() #quantile(quantile,interpolation='nearest').copy()
    ...
    df_chat_eta2[output].loc[list(df_chat_eta2['count'][df_chat_eta2['count'] == 0].index.values)] = df1_update.groupby(['eta2','c_hat']).max().loc[list(df_chat_eta2['count'][df_chat_eta2['count'] == 0].index.values)]['E_cor']
    ...
    for i in range(len(df_chat_eta2)):
        ...
        if np.isnan(out1) or out1 == 0:
            ...
            if combination == True:
                ...
                max_ind = df1_update.query('E_cor >= @threshold_E').groupby(['eta2','c_hat']).max().loc[df_chat_eta2.iloc[i].name]
                min_ind = df1_update.query('E_cor >= @threshold_E').groupby(['eta2','c_hat']).min().loc[df_chat_eta2.iloc[i].name]
                max_c1 = max_ind.loc['c1']
                max_c2 = max_ind.loc['c2']
                min_eta1 = min_ind.loc['eta1']
