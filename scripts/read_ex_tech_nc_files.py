...
def plot_existingtechs_result(N_select='37', Ns=['37','74']):
    import pandas as pd
    import numpy as np
    import pypsa
    ...
    statement = True
    for N in Ns:
        if not os.path.isfile('results/Emerging_storage_N' + str(N) + '.csv'):
            print('.csv file for N=',N,' does not exist')
            statement = False
            break
        else:
            print('.csv file for N=',N,' already exists')
            
    if not statement:
        PyPSA_Eur_Sec_v = '0.4.0'
        x_dic = {'0.4.0':["X Discharge",'X Charge','X']}
        files = glob('networks/ex_techs/*.nc')
        csv = pd.read_csv('data/storage_techs_cost_USD.csv',sep=';',index_col=0)
        ...
        for N in Ns:
            ...
            for file in files:
                ...
                i += 1
            # print(X_EU)
            X_EU.to_csv('results/Emerging_storage_N' + str(N) + '.csv')
            X_EUs[N] = X_EU
    
    else:
        X_EUs = {}
        for N in Ns:
            X_EUs[N] = pd.read_csv('results/Emerging_storage_N' + str(N) + '.csv',index_col=0)
    ...
    output = 'E [MWh_e]'
    # output = 'G_d [MW_e]'
    output_ylab_dic = {'E [MWh_e]':'Energy capacity ' + '$E$' + ' [GWh]', # + r'$_e$' + ']',
                  'G_d [MW_e]':'Power capacity ' + '$G_d$' + ' [GW]',} # + r'$_e$' + ']',}
    output_dic = {'E [MWh_e]':'E',
                  'G_d [MW_e]':'G_d',}
    wi = 0.1
    tech = {}
    fig,ax = plt.subplots(figsize=[7,12])
    neg_area_leg = ax.fill_between([1e-2,1],[3.3, 3.3],[-0.3, -0.3],facecolor="none", edgecolor = 'k', hatch='//',label='Negligible',zorder=2)
    count_i = 0
    cols = list(X_EUs[N_select].columns)
    cols.sort()
    leg = {}
    for tech in techs:
        count_j = 0
        for col in cols:
            if tech == col.split('-')[0]:
                ...
                y_val = X_EUs[N_select][col][output]/1e3
                ...
                leg[col.split('-')[1]] = ax.barh(0.5*count_i+wi*count_j-wi,y_val,height=wi,color=color_dic[col.split('-')[1]],zorder=zorder)
                count_j += 1
    ...
    legend = list(leg.values())
    legend.reverse()
    ax.set_xlim([1e-2,2500])
    ax.set_ylim([-0.3,3.3])
    ax.axvline(2000,color='grey',linewidth=3)
    ax.legend([neg_area_leg] + legend,[r'$E\approx 0$','SC3', 'SC2', 'SC1'],prop={'size':18},frameon=True)
    ...
    fig.savefig('figures/Existing_techs_output_' + output_dic[output] + '_N' + N_select + '.png', dpi=300,
                  bbox_inches="tight")
    return fig