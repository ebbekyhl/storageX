...
def prepare_network(n, eta_c,eta_d,c_c,c_d,c_hat,solve_opts=None):
    RTE = eta_c*eta_d 
    lambda1_o = 57
    lambda2_o = 175
    XCOS = (c_c/lambda1_o + c_d/lambda2_o + c_hat) / RTE
    
    if (RTE > 1) or (XCOS > 46):
        return n

    n_chat = n.stores.query('carrier == "X"').capital_cost[0] # annualized cost
    n_c_c = n.links.query('carrier == "X Charge"').capital_cost[0] # annualized cost
    n_c_d = n.links.query('carrier == "X Discharge"').capital_cost[0] # annualized cost

    tau = float(snakemake.wildcards.sloss)
    deltaE = 1 - np.exp(-1/(24.*tau))

    X = n.stores.carrier == 'X'    
    n.stores.loc[X,'capital_cost'] = n_chat*float(snakemake.wildcards.storage_c)
    n.stores.loc[X,'standing_loss'] = deltaE
    
    X_charge = n.links.carrier == 'X Charge'
    n.links.loc[X_charge,'capital_cost'] = n_c_c*float(snakemake.wildcards.charge_c)
    n.links.loc[X_charge,'efficiency'] = eta_c

    X_discharge = n.links.carrier == 'X Discharge'
    n.links.loc[X_discharge,'capital_cost'] = n_c_d*float(snakemake.wildcards.discharge_c)
    n.links.loc[X_discharge,'efficiency'] = eta_d
...
# ---------------------------------------------------------------------------------------------------------------------------------------
# ---------------------------------------- Extra functionality in lopf solver -----------------------------------------------------------
# ---------------------------------------------------------------------------------------------------------------------------------------
...
def solve_network(n,eta_c,eta_d,c_c,c_d,c_hat,config=None, solver_log=None, opts=None):

    saveduals = True
    
    RTE = eta_c*eta_d 
    XCOS = ((c_c+c_d)/10 + c_hat) / RTE
    
    if RTE > 1: #(RTE > 1) or (XCOS > 600):
        saveduals = False
        return n,saveduals
...
if __name__ == "__main__":
    ...
    print('c_d = ',c_d)
    c_hat = 20*float(snakemake.wildcards.storage_c)
    print('c_hat = ',c_hat)
    tau = float(snakemake.wildcards.sloss)
    print('tau = ',tau)
    ...
    with memory_logger(filename=getattr(snakemake.log, 'memory', None), interval=30.) as mem:
        ...
        n = prepare_network(n,eta_c,eta_d,c_c,c_d,c_hat)

        print(n)

        n,saveduals = solve_network(n,eta_c,eta_d,c_c,c_d,c_hat)

        print(n)

        if saveduals == True:
            path = snakemake.output[0]
            n.name = os.path.relpath(os.path.normpath(path))
            n.dual_path = n.name[:-2]+'p'
            pickle.dump((n.duals,n.dualvalues),open(n.dual_path, "wb" ))
            print('duals saved')
