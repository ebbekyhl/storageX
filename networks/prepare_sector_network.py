...
def correct_phs_capacity(n):
    p_nom = np.array([7,0.4,1.3,0.9,4.1,1.2,8.4,6.9,5,2.6,0.7,0.5,0.4,8.8,0.9,1.3,0.3,1.7,2.6,0.6,0.1,0.2,0.8])*1e3
    max_hours = [32.7,6,4.4,7.9,136.0,5.1,5.7,11.0,15.1,9.6,6.8,8.2,6.2,8.7,12.0,3.8,314.5,4.5,31.9,6,801.0,2.6,3.5]
    # Update following nodes in the following order:
    # AT0, BA0, BE0, BG0, CH0, CZ0, DE0, ES0, FR0, GB5, GR0, HR0, IE4, IT0, LT6, LU0, NO3, PL0, PT0, RS0, SE3, SI0, SK0
    phs = n.storage_units.carrier == 'PHS'
    n.storage_units.loc[phs,'max_hours'] = max_hours
    n.storage_units.loc[phs,'p_nom'] = p_nom
    n.storage_units.loc[phs,'p_nom_opt'] = p_nom
...
def add_generation(network):
    ...
    for generator,carrier in conventionals.items():
        ...
        if "EU " + carrier not in network.buses.index:
            network.madd("Bus",
                        ["EU " + carrier],
                        location="EU",
                        carrier=carrier)
        ...
        #use madd to get carrier inserted
        if "EU " + carrier + " Store" not in network.generators.index:
            network.madd("Store",
                        ["EU " + carrier + " Store"],
                        bus=["EU " + carrier],
                        e_nom_extendable=True,
        ...
        if "EU " + carrier not in network.generators.index:
...
def add_storage(network,opts):
    ...
    #              capital_cost=costs.at["fuel cell","fixed"]*costs.at["fuel cell","efficiency"], #NB: fixed cost is per MWel which is why we multiply by the efficiency
    #              lifetime=costs.at['fuel cell','lifetime'])

    eta_c = 0.5 #*float(snakemake.wildcards.charge_e)
    # c_c = 350 #*float(snakemake.wildcards.charge_c)
    eta_d = 0.5 #*float(snakemake.wildcards.discharge_e)
    # c_d = 350 #*float(snakemake.wildcards.discharge_c)
    # chat = 20 #*float(snakemake.wildcards.storage_c)
    tau = 30 #float(snakemake.wildcards.sloss)
    ...
    network.madd("Link",
                ...
                carrier = "X Charge",
                efficiency = eta_c,
                capital_cost = costs.at["X Charge","fixed"], #float(snakemake.wildcards.charge_c)*costs.at["X Charge","fixed"],
                lifetime = costs.at['X Charge','lifetime'])
    ...
    network.madd("Link",
                ...
                carrier ="X Discharge",
                efficiency = eta_d,
                capital_cost = costs.at["X Discharge","fixed"]*eta_d, #float(snakemake.wildcards.discharge_c)*costs.at["X Discharge","fixed"]*eta_d, #NB: fixed cost is per MWel which is why we multiply by the efficiency
                lifetime = costs.at['X Discharge','lifetime'])
    ...
    network.madd("Store",
                ...
                e_cyclic=True,
                carrier="X",
                capital_cost=costs.at["X Store","fixed"], #float(snakemake.wildcards.storage_c)*costs.at["X Store","fixed"],
                standing_loss = deltaE,
                lifetime = costs.at['X Store','lifetime']) 
...
def remove_h2_links(n):
    n.links.drop(n.links.index[n.links.carrier.isin(["SMR","SMR CC","Sabatier","helmeth"])],inplace=True)
...
if __name__ == "__main__":
    ...
    if 'snakemake' not in globals():
        ...
        snakemake = MockSnakemake(
            wildcards=dict(network='elec', simpl='', clusters='45', lv='1.0',
                           opts='', planning_horizons='2030',
                           sector_opts='Co2L0-168H-H2 Electrolysis+c0.5'),

            input=dict( network='../pypsa-eur-0.3.0/pypsa-eur/networks/{network}_s{simpl}_{clusters}_ec_lv{lv}_{opts}.nc',
                        energy_totals_name='../pypsa-eur-0.3.0/pypsa-eur-sec/resources/energy_totals.csv',
                        co2_totals_name='../pypsa-eur-0.3.0/pypsa-eur-sec/resources/co2_totals.csv',
                        transport_name='../pypsa-eur-0.3.0/pypsa-eur-sec/resources/transport_data.csv',
                	    traffic_data = "../pypsa-eur-0.3.0/pypsa-eur-sec/data/emobility/",
                        biomass_potentials='../pypsa-eur-0.3.0/pypsa-eur-sec/resources/biomass_potentials.csv',
                        timezone_mappings='../pypsa-eur-0.3.0/pypsa-eur-sec/data/timezone_mappings.csv',
                        heat_profile="../pypsa-eur-0.3.0/pypsa-eur-sec/data/heat_load_profile_BDEW.csv",
                        costs="../technology-data/outputs/costs_{planning_horizons}.csv",
                	    h2_cavern = "../pypsa-eur-0.3.0/pypsa-eur-sec/data/hydrogen_salt_cavern_potentials.csv",
                        profile_offwind_ac="../pypsa-eur-0.3.0/pypsa-eur/resources/profile_offwind-ac.nc",
                        profile_offwind_dc="../pypsa-eur-0.3.0/pypsa-eur/resources/profile_offwind-dc.nc",
                        busmap_s="../pypsa-eur-0.3.0/pypsa-eur/resources/busmap_{network}_s{simpl}.csv",
                        busmap="../pypsa-eur-0.3.0/pypsa-eur/resources/busmap_{network}_s{simpl}_{clusters}.csv",
                        clustered_pop_layout="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/pop_layout_{network}_s{simpl}_{clusters}.csv",
                        simplified_pop_layout="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/pop_layout_{network}_s{simpl}.csv",
                        industrial_demand="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/industrial_energy_demand_{network}_s{simpl}_{clusters}.csv",
                        heat_demand_urban="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/heat_demand_urban_{network}_s{simpl}_{clusters}.nc",
                        heat_demand_rural="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/heat_demand_rural_{network}_s{simpl}_{clusters}.nc",
                        heat_demand_total="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/heat_demand_total_{network}_s{simpl}_{clusters}.nc",
                        temp_soil_total="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/temp_soil_total_{network}_s{simpl}_{clusters}.nc",
                        temp_soil_rural="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/temp_soil_rural_{network}_s{simpl}_{clusters}.nc",
                        temp_soil_urban="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/temp_soil_urban_{network}_s{simpl}_{clusters}.nc",
                        temp_air_total="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/temp_air_total_{network}_s{simpl}_{clusters}.nc",
                        temp_air_rural="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/temp_air_rural_{network}_s{simpl}_{clusters}.nc",
                        temp_air_urban="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/temp_air_urban_{network}_s{simpl}_{clusters}.nc",
                        cop_soil_total="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/cop_soil_total_{network}_s{simpl}_{clusters}.nc",
                        cop_soil_rural="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/cop_soil_rural_{network}_s{simpl}_{clusters}.nc",
                        cop_soil_urban="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/cop_soil_urban_{network}_s{simpl}_{clusters}.nc",
                        cop_air_total="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/cop_air_total_{network}_s{simpl}_{clusters}.nc",
                        cop_air_rural="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/cop_air_rural_{network}_s{simpl}_{clusters}.nc",
                        cop_air_urban="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/cop_air_urban_{network}_s{simpl}_{clusters}.nc",
                        solar_thermal_total="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/solar_thermal_total_{network}_s{simpl}_{clusters}.nc",
                        solar_thermal_urban="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/solar_thermal_urban_{network}_s{simpl}_{clusters}.nc",
                        solar_thermal_rural="../pypsa-eur-0.3.0/pypsa-eur-sec/resources/solar_thermal_rural_{network}_s{simpl}_{clusters}.nc",
                	    retro_cost_energy = "../pypsa-eur-0.3.0/pypsa-eur-sec/resources/retro_cost_{network}_s{simpl}_{clusters}.csv",
                        floor_area = "../pypsa-eur-0.3.0/pypsa-eur-sec/resources/floor_area_{network}_s{simpl}_{clusters}.csv"
            ),
    ...
    #convs = snakemake.config["conventional_generation"]

    # network_name = snakemake.wildcards.sector_opts + '-H2 Electrolysis+e' + snakemake.wildcards.charge_e + '-H2 Electrolysis+c' + snakemake.wildcards.charge_c + '-H2 Fuel Cell+e' + snakemake.wildcards.discharge_e + '-H2 Fuel Cell+c' + snakemake.wildcards.discharge_c + '-H2 Store+c' + snakemake.wildcards.storage_c + '-standingloss+' + snakemake.wildcards.sloss
    
    network_name = 'ref_' + snakemake.wildcards.sector_opts #+ '-X Charge+e' + snakemake.wildcards.charge_e + '-X Charge+c' + snakemake.wildcards.charge_c + '-X Discharge+e' + snakemake.wildcards.discharge_e + '-X Discharge+c' + snakemake.wildcards.discharge_c + '-X Store+c' + snakemake.wildcards.storage_c + '-standingloss+' + snakemake.wildcards.sloss
    
    print(network_name)
    opts = network_name.split('-') #snakemake.input.split('-') #snakemake.wildcards.sector_opts.split('-')   
    ...
    #simplified_pop_layout = pd.read_csv('resources/pop_layout_elec_s.csv',index_col=0)
    ...
    for o in opts:
        if "+" in o:
            ...
            if oo[0].startswith(tuple(suptechs)):
                carrier = oo[0]
                attr_lookup = {"p": "p_nom_max", "c": "capital_cost", "e": "efficiency"}
                print(oo)
                ...
                if carrier == "AC":  # lines do not have carrier
                    ...
                    print("changing", attr ,"for lines by factor",factor)
